...
const DoubleButton: React.FC<DoubleButtonProps> = ({
  ...
  let heightAnimatedValue = useRef(0).current;
  useEffect(() => {
    buttonHeight.addListener(value => {
      heightAnimatedValue = Number(value.value);
    });
  }, []);
  useEffect(() => {
  ...
  // 结束长按左/右侧按钮
  const handleAnimatedOut = (direction: 'left' | 'right', heightValue: number) => {
    console.log('aaaaaaaaaaaaa');

    if (direction === 'left') {
      leftAnimatedReset(heightValue * 0.2).start();
      handleLeftOut();
    } else {
      rightAnimatedReset(heightValue * 0.2).start();
      handleRightOut();
    }
  };
  // 长按左/右侧按钮时间到触发
  ...
  const button = (direction: 'left' | 'right') => {
    ...
    return (
      <TouchableOpacity
        ...
        onPressOut={() =>
          activeButton !== direction && handleAnimatedOut(direction, heightAnimatedValue)
        }
      >
        <Animated.View
          style={[
